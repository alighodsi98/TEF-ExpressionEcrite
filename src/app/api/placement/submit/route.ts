...
import { correctExercise, evaluatePlacement, type CorrectionResult } from "@/lib/ai";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "placement": "Évaluation du positionnement",
};
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // Phase 1: Corrections in parallel
        const results = await Promise.allSettled(
          tasks.map(async (task) => {
            ...
            const corr = await correctExercise({
              ...
              topic: task.section === "A" ? sectionA.topic : sectionB.topic,
        ...
        if (successCount === 0) {
          console.error(`[placement/submit] All ${results.length} corrections failed. Last error: ${lastError}`);
        ...
        completed++;
        send({ type: "done_step", key: "placement", completed, total: totalSteps, label: STEP_LABELS["placement"] });
              ...
              create: [
                {
                  ...
                  corrections: {
                    create: [correctionCreateData("realistic", corrA)],
                  },
                },
                {
                  section: "B",
                  ...
                  corrections: {
                    create: [correctionCreateData("realistic", corrB)],
                  },
                },
              ],
            },
        ...
        await db.userProfile.update({
          ...
          data: {
            placementDone: true,
            placementMode: "realistic",
            levelSectionA: placement.levelSectionA,
            levelSectionB: placement.levelSectionB,
            levelOverallNclc: placement.levelOverallNclc,
            levelOverallCecr: placement.levelOverallCecr,
          },
        });

        send({ type: "done", data: { sessionId: session.id, placement, corrections: { realistic: { A: corrA, B: corrB } } } });
      } catch (e) {
